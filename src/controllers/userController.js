...
const alarm_immediately = async (data) => {
  let datetimeUTC = moment(data.date).local().utcOffset().format('YYYY-MM-DD HH:mm:ss');
  let datetimeLocal = moment.utc(data.date).local().utcOffset().format('YYYY-MM-DD HH:mm:ss');
